...
Updated 2026-09-04: a second batch was ingested on 2026-08-25, taking the cache to **947/956 (99.1%)** — 544 of those entries are now locally supplied. Note the provenance tag is split: the 449 from the first run carry `source_origin="kolja"` (the tag the script wrote at the time), the 95 from the second run carry `source_origin="collaborator"` (the current `SOURCE_ORIGIN` constant). The rename was never backfilled into the cache, so any provenance query has to match both.

### 12. Split `data/` into sources vs cache (`data/sources/`, `data/cache/`)
The flat `data/` dir mixed ground-truth inputs with rebuildable outputs, so "what is safe to delete and regenerate" was implicit. Reorganized into `data/sources/` (publications CSV, `.enl`, proposal PDF, and a collaborator's PDFs under `data/sources/pdfs/` — never regenerated) and `data/cache/` (everything `build.py` produces). Path constants across the build scripts, the runtime modules (`search.py`, `semantic_search.py`, `graph.py`, `corpus_map.py`, `server.py`, `app.py`), and `build.py`'s dependency graph now resolve inputs from `sources/` and outputs to `cache/`; the 449 locally-ingested (`source_origin="kolja"`) provenance urls were rewritten `local:data/pdfs/…` → `local:data/sources/pdfs/…` (all verified to resolve). During the move the 70 `duplicate_` PDFs were deleted and the 8 off-corpus PDFs + the `10.1039/d2cc03286d` HTML stub were quarantined to `data/sources/pdfs/_unmatched/` pending a corpus-scope decision. `data/` is gitignored, so the tracked change is code + docs only; `build.py` fails loud naming the missing file under `data/sources/` if a required input is absent. 26/26 tests green.
...
Two more items from the 03.09.26 meeting's owner-tagged next steps. `app.py` shows a "💬 Feedback" popover under every assistant answer — free-text bug report or general feedback (not a rating; a thumbs widget was tried first but the meeting note and a follow-up ask both wanted bug/feedback text, not a score) — submitted via an `st.form` (clears on submit) and appended as JSONL to `data/feedback/feedback.jsonl` (question, answer, model, category, text). No external service, matching the single-user setup.
